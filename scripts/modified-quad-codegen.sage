...
b = 2^(log2q//k)                                                 # we need q1 to be congruent to 5 modulo 8
p = b^2 + 1 
...
full_size = kmsis * d * (log2q - D) + (l+ lext)* d * log2q   
challenge = ceil(log(2*omega+1,2)) * d 
short_size1 = m1 * d * (ceil(log(stdev1,2) + 2.57)) + (m2 - kmsis) * d * (ceil(log(stdev2,2) + 2.57))
hint = 2.25 * kmsis * d

enablePrint()
printv(f"auto-generated by modified-quad-codegen.sage from {params_file}.")
printv(f"")


if not (kmlwe >= 0 and kmlwe == m2 - kmsis - l - lext):
    err("protocol not simulatable")

printv(
    f"the commitment scheme is hiding under 2. MLWE({kmlwe},{kmsis+l+ lext}) with sd={nstr(sigma_2, prec)})") 
printv(
    f"protocol is simulatable under Hint-MLWE implied by MLWE({kmlwe},{kmsis+l+ lext}) with sd={nstr(varsigma_2, prec)})") 
...
printv(f"")
printv(f"Proof size of the modified quadratic proof ")
