...
lmbda = 2 * ceil(param_sec/(2*log2q))
...
if not (kmlwe >= 0 and kmlwe == m_2 - kmsis - lext):
...
static const rf_abdlop_params_t {name}_quad_eval = {{{{ {name}_ring, {name}_dcomp, {name}_b, {m_1}, {m_2}, {l}, {lext}, {kmsis}, {name}_Bsq, {omega}, {omega_bits}, {eta}, {name}_sigma_1, {ceil(mp.log(sigma_1,2))}, {name}_sigma_2, {ceil(mp.log(sigma_2,2))}, {name}_frak_s1, {ceil(mp.log(frak_s_1,2))}, {name}_frak_s2, {ceil(mp.log(frak_s_2,2))}}}}};
static const rf_abdlop_params_t {name}_quad_many = {{{{ {name}_ring, {name}_dcomp, {name}_b, {m_1}, {m_2}, {l+lmbda/2}, {1}, {kmsis}, {name}_Bsq, {omega}, {omega_bits}, {eta}, {name}_sigma_1, {ceil(mp.log(sigma_1,2))}, {name}_sigma_2, {ceil(mp.log(sigma_2,2))}, {name}_frak_s1, {ceil(log(frak_s_1,2))}, {name}_frak_s2, {ceil(log(frak_s_2,2))}}}}};
static const rf_quad_eval_params_t {name} = {{{{ {name}_quad_eval, {name}_quad_many, {lmbda}}}}};
"""
